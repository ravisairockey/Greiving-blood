...
void FEdGraphParameterFormatter::FormatNode(UEdGraphNode* InNode)
{
	...
	if (FBAUtils::IsNodeImpure(RootNode))
	{
		...
		{
			int Baseline = FMath::RoundToInt(FBAUtils::GetPinPos(GraphHandler, Pin).Y + UBASettings::Get().ParameterVerticalPinSpacing);
...
bool FEdGraphParameterFormatter::DoesHelixingApply()
{
	...
	while (NodeQueue.Num() > 0)
	{
		...
		{
			const bool bInputLinkedToMultipleNodes = LinkedToNodesInput.Num() >= UBASettings::Get().DisableHelixingPinCount;
				...
				if (ParentPin != nullptr && MyPin != nullptr)
				{
					...
					if (ParentNode != CurrentInfo->GetParentNode())
					{
						...
						if (bTakeNewParent)
						{
							...
							TSet<UEdGraphNode*> TempChildren;
							CurrentInfo->MoveChildren(CurrentInfo, GraphHandler, Padding.X, TempChildren);
						}
						else
						{
...
int32 FEdGraphParameterFormatter::GetChildX(const FPinLink& Link, const EEdGraphPinDirection Direction) const
{
	...
	{
		NewNodePos = ParentBounds.Left - ChildBounds.GetSize().X - FMath::Max(1, Padding.X);
	}
	else
	{
		NewNodePos = ParentBounds.Right + FMath::Max(1, Padding.X);
	}
...
void FEdGraphParameterFormatter::MoveBelowBaseline(TSet<UEdGraphNode*> Nodes, int Baseline)
{
	...
	const int Top = FMath::RoundToInt(Bounds.Top);
	if (Baseline > Top)
	{
		const int Delta = Baseline - Top;
